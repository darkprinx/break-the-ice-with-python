...
for i in word:
    if i.isalpha(): # returns True if alphabet
        ...
        digit += 1
print(f"LETTERS {letter}\n{digit}") # two different types of formatting method is shown in both solution
```
---
```python
...
l = list(input())
print("Letters: " + str(len(list(filter(check_letter,l)))))
print("Digits: " + str(len(list(filter(check_number,l)))))
```
...
'''
def count_letter_digits(text):
    letters_count = len([i for i in text if i.isalpha()])
    digits_count = len([i for i in text if i.isdigit()])
    result = f'LETTERS {letters_count}\nDIGITS {digits_count}'
    return result


text = input()
print(count_letter_digits(text))
```
